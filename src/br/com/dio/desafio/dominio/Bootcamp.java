    ...
    public void gerarCertificado(Dev dev) {
        ...
        if (idTodosDevs().contains(dev.getId())) {
            if (dev.conteudosInscritos.isEmpty()) {
                ...
                System.out.printf(
                        "Certificamos que %s em " + formatarData.format(dataAtual)
                                + ", concluiu o bootcamp com com carga horaria de de horas \n",
                        dev.getNome(), this.getNome(), this.getCargaHoraria());
            } else {
                System.out.println("Para gerar o seu certificado, voce precisa concluir os seguintes conteudos: "
